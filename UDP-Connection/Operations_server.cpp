...
void handle_client(int server_socket) {
    ...
    while(1){
        ...
        recvfrom(server_socket, buf, sizeof(buf), 0, (struct sockaddr *) &client_address, &client_address_len);
        ...
        // result = num1 + num2;
        if(num1%2){
            char message[] = "The first number is odd";
            sendto(server_socket, &message, sizeof(message), 0, (struct sockaddr *) &client_address, client_address_len);
        }
        else{
            char message[] = "The first number is even";
            sendto(server_socket, &message, sizeof(message), 0, (struct sockaddr *) &client_address, client_address_len);
